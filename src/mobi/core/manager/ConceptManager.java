...
public class ConceptManager implements Serializable {
	...
	private void addConcept(SymmetricRelation symmetricRelation) {
		String directNameRelation = this
				.getDirectNameOfRelation(symmetricRelation);
		String inverseNameRelation = this
		...
		if (!this.allSymmetricRelations.containsKey(directNameRelation)
			...
			this.allSymmetricRelations.put(symmetricRelation.getUri(),
					symmetricRelation);
		}
	}
	...
	private void addConcept(EquivalenceRelation equivalenceRelation) {
		...
		String inverseNameRelation = this
				.getInverseNameOfRelation(equivalenceRelation);
	...
	private void addConcept(InheritanceRelation inheritanceRelation) {
		...
		String inverseNameRelation = this
				.getInverseNameOfRelation(inheritanceRelation);
	...
	private String getDirectNameOfRelation(Relation r) {
		...
		if (r.getType() == Relation.INHERITANCE
	...
	private String getInverseNameOfRelation(Relation r) {
		...
		if (r.getType() == Relation.INHERITANCE
				|| r.getType() == Relation.EQUIVALENCE)
			nameMobiRelation = r.getClassB().getUri() + "_" + r.getUri() + "_"
					+ r.getClassA().getUri();
		else if (r.getType() == Relation.SYMMETRIC_COMPOSITION)
			nameMobiRelation = r.getClassB().getUri() + "_"
					+ ((SymmetricRelation) r).getName() + "_"
					+ r.getClassA().getUri();
		else if (r.getType() == Relation.UNIDIRECIONAL_COMPOSITION)
			nameMobiRelation = "";
			...
			nameMobiRelation = r.getClassB().getUri() + "_"
					+ ((CompositionRelation) r).getNameB() + "_"
					+ r.getClassA().getUri();

		return nameMobiRelation;
	}

	public void linkInstances(String instanceVector, String classURI)
			throws Exception {
	...
	public void linkInstances(Set<Instance> instanceSet, Class mobiClass)
			throws Exception {
	...
	public void isOneOf(String instanceURI, String classURI)
			throws ExceptionURI {
	...
	public void isOneOf(Instance instance, Class mobiClass) throws ExceptionURI {
		...
		if (this.allInstanceClassRelation.get(instance.getUri()) == null)
			this.allInstanceClassRelation.put(instance.getUri(),
					new HashSet<Class>());
		if (this.allClassInstanceRelation.get(mobiClass.getUri()) == null)
			this.allClassInstanceRelation.put(mobiClass.getUri(),
					new HashSet<Instance>());

		if (!this.allInstanceClassRelation.get(instance.getUri()).contains(
				mobiClass)) {
			this.allInstanceClassRelation.get(instance.getUri()).add(mobiClass);
		} else {
			throw new ExceptionURI(
					"This instance is already bound to this class!");
		}
		if (!this.allClassInstanceRelation.get(mobiClass.getUri()).contains(
				instance)) {
			this.allClassInstanceRelation.get(mobiClass.getUri()).add(instance);
		} else {
			throw new ExceptionURI(
					"This class is already bound to this instance!");
		}
	...
	public void setAllCompositionRelations(
			HashMap<String, CompositionRelation> allCompositionRelations) {
	...
	public CompositionRelation getCompositionRelation(
	...
	public void removeInstanceRelation(Relation relation, Instance instanceA,
		...
		r.getInstanceRelationMapA().get(instanceA.getUri()).removeInstance(
				instanceB);
		r.getInstanceRelationMapB().get(instanceB.getUri()).removeInstance(
				instanceA);

		if (r.getInstanceRelationMapA().get(instanceA.getUri())
				.getInstanceMapSize() == 0)
			r.getInstanceRelationMapA().remove(instanceA.getUri());
		if (r.getInstanceRelationMapB().get(instanceB.getUri())
				.getInstanceMapSize() == 0)
	...
	public void setAllSymmetricRelations(
			HashMap<String, SymmetricRelation> allSymmetricRelations) {
	...
	public SymmetricRelation getSymmetricRelation(
	...
	public void setAllInheritanceRelations(
			HashMap<String, InheritanceRelation> allInheritanceRelations) {
	...
	public void setAllEquivalenceRelations(
			HashMap<String, EquivalenceRelation> allEquivalenceRelations) {
	...
	public EquivalenceRelation getEquivalenceRelation(
	...
	public String getInversePropertyName(String nameObjectProperty) {
		for (CompositionRelation cr : this.allCompositionRelations.values()) {
			if (cr.getNameA() != null
					&& this.getPropertyName(cr.getNameA(), cr.getClassA(),
							cr.getClassB()).equals(nameObjectProperty)
					&& cr.getNameB() != null)
				return this.getPropertyName(cr.getNameB(), cr.getClassB(), cr
						.getClassA());

			if (cr.getNameB() != null
					&& this.getPropertyName(cr.getNameB(), cr.getClassB(),
							cr.getClassA()).equals(nameObjectProperty))
				return this.getPropertyName(cr.getNameA(), cr.getClassA(), cr
						.getClassB());
		}
	...
	public void setAllRemovedConcepts(
			HashMap<String, Concept> allRemovedConcepts) {
